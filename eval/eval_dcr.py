...
if __name__ == '__main__':
    ...
    for i in num_col_idx:
        if real_data[i].max() - real_data[i].min() == 0:
            to_append = 1
        else:
            to_append = real_data[i].max() - real_data[i].min()
        num_ranges.append(to_append) 
    ...
    encoder.fit(np.concatenate((cat_real_data_np,cat_test_data_np),axis=0))
