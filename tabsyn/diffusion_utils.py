...
def sample(net, num_samples, dim, num_steps = 50, device = 'cuda:0', controlnet=False, model = None, cf = None, weight = 1.0):
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            x_next = sample_step(net, num_steps, i, t_cur, t_next, x_next, controlnet, model, cf, weight)

    return x_next

def sample_step(net, num_steps, i, t_cur, t_next, x_next, controlnet=False, model = None, cf = None, weight = 1.0):
    ...
    # Euler step.
    denoised_uncod = net(x_hat, t_hat).to(torch.float32)
    denoised_cod = net(x_hat, t_hat, controlnet, model, cf).to(torch.float32)
    denoised = denoised_uncod + weight * (denoised_cod - denoised_uncod)
    d_cur = (x_hat - denoised) / t_hat
    x_next = x_hat + (t_next - t_hat) * d_cur

    # Apply 2nd order correction.
    if i < num_steps - 1:
        denoised_uncod = net(x_next, t_next).to(torch.float32)
        denoised_cod = net(x_next, t_next, controlnet, model, cf).to(torch.float32)
        denoised = denoised_uncod + weight * (denoised_cod - denoised_uncod)
...
class EDMLoss:
    ...
    def __call__(self, denoise_fn, data, controlnet=False, model = None, xf = None, weight = 1):#self.denoise_fn = Precond(denoise_fn, hid_dim)
        ...
        D_yn = denoise_fn(y + n, sigma, controlnet, model, xf, weight)
    
        target = y
        loss = edm_weight.unsqueeze(1) * ((D_yn - target) ** 2)
        return loss.mean(-1)
